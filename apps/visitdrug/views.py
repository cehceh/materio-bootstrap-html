        ...
        if form.is_valid():
            ...
            if match_row < 3:
                ...
                return HttpResponseRedirect(
                    reverse("visitdrug:save_medicine", args=(pat_id, vis_id))
                )  # that's it()
            else:
                messages.success(
                    request,
                    "You can't add more drugs, You are in a DEMO mode, For full version contact Eng. Amr Aly on 01067174141",
                )
            # else:
            #     messages.success(
...
def edit_medicine(request, patient_id, visit_id, id):  # Making Update to a Medicine
    ...
    visitid = (
        Visits.objects.values("id").filter(id=visit_id).first()
    )  # out put is {'id':its value}
    visdate = (
        Visits.objects.values("visitdate").filter(id=visit_id).first()
    )  # out put is {'visitdate':its value}
    ...
    first_rec = (
        Medicine.objects.values("patient").filter(visit=vis_id, patient=pat_id).first()
    )
...
def add_new(request, patient_id, visit_id):
    ...
    # print('q = ' + str(q))
    table = MedicineTable(
        Medicine.objects.filter(visit=visit_id, patient=patient_id).order_by("-id"),
        exclude="patient",
    )
    # table = MedicineTable(
    ...
    qs = (
        Visits.objects.values("patient_id")
        .filter(id=visit_id, patient=patient_id)
        .first()
    )
    # qsvalue = qs['patient']
        ...
        if form.is_valid():
            ...
            if match:  # match_row < 2:#(vis == vis_id) and (pat == pat_id)
                ...
                if match_row < 3:
                    ...
                    return HttpResponseRedirect(
                        reverse("visitdrug:add_new", args=(pat_id, vis_id))
                    )  # that's it()
                else:
                    messages.success(
                        request, "Drugs reach to its limit, You are in a demo mode"
                    )
            else:
                messages.success(
                    request,
