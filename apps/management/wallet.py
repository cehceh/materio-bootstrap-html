...
def add_wallet(request):
    ...
    if request.accepts('*/*') and request.method == "POST":
        ...
        if wallet_name == '0' and (wallet_new_name == '0' or wallet_new_name == '') :
            data['msg'] = "Please Choose From the Wallet Name or From the Wallet New Name"
            data['type'] = "error"
        ...
        if (account_name == '' and  mobile == '0') or (account_name == '' and  mobile == '') :
        ...
        if form.is_valid():
            ...
            CreateWallet.objects.filter(id=save_form.id).update(code='wal-'+str(save_form.id))
...
def edit_wallet(request, id):
    ...
    if request.accepts('*/*') and request.method == "POST":
        ...
        if wallet_name in ['4', '5']:
            # Wallet names 4 or 5 require an account name, and mobile validation is not needed
            if account_name == '' or account_name == None:
                data['msg'] = "Please enter an account name for Wallets Instapay or Paymob"
                data['type'] = "error"
                return JsonResponse(data)
        elif wallet_name == '1':
            # Wallet name 1 requires the mobile number to start with "010"
            if not mobile.startswith("010"):
                data['msg'] = "Mobile number for Wallet Vodafone should start with '010'"
                data['type'] = "error"
                return JsonResponse(data)
        elif wallet_name == '2':
            # Wallet name 2 requires the mobile number to start with "011"
