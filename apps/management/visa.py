...
def add_visa(request):
    ...
    if request.accepts("*/*") and request.method == "POST":
        ...
        if form.is_valid():
            ...
            CreateVisa.objects.filter(id=save_form.id).update(
                code="visa-" + str(save_form.id)
            )
    ...
    context = {"form": form, "title": "Create Visa"}
...
def edit_visa(request, id):
    ...
    context = {"form": form, "qs": qs, "visa_code": visa_code, "title": "Edit Visa"}
...
def create_ajax_visa_message(request):
    ...
    if request.is_ajax() and request.method == "POST":
        ...
        visa_name = request.POST.get("visa_name")
        match = (
            CreateVisa.objects.select_related("user").filter(name=visa_name).exists()
        )
        if not match:
            if form.is_valid():
...
def visa_table(request):
    table_qs = (
        CreateVisa.objects.select_related("user")
